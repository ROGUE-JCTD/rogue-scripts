...
var TestModule = (function() {
  ...
  var config = {
    ...
    // when disabled, the view will not be moved at all while operations such as createFeature still drop points on different
    // parts of the map. This will essentially not task the server with wms calls.
    moveToViewEnabled: false,
  ...
  function run() {
    //-- debug to make sure only one 'task' is running
    timerCountActive -= 1;
    if (timerCountActive > 0) {
      alert('Debug Error: timerCountActive greater than 1! ');
    }
  ...
  function setTimer() {
    // once the script has been stopped, even though the timeout is cleared, if a task is being executed, it will still
    // set a new timer. the stopped check here prevents this special case.
    if (!stopped) {
  ...
  p.start = function(frequency) {
    ...
    console.log('====[ startTest. frequency: ', config.frequency, ', config: ', config, ', retrieving existing layer features...');
